...
import com.turkcell.payment.entity.OutboxEvent;
import com.turkcell.payment.repository.OutboxEventRepository;
...
class OutboxEventPublisherTest {
    ...
    void marksEventPublishedWhenSendSucceeds() {
        ...
        when(streamBridge.send(eq("paymentEvents-out-0"), any(Message.class))).thenReturn(true);
    ...
    void incrementsRetryCountAndKeepsPendingWhenSendReturnsFalse() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("paymentEvents-out-0"), any(Message.class))).thenReturn(false);
