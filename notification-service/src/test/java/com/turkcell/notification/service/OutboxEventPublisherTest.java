...
class OutboxEventPublisherTest {
    ...
    private OutboxEvent pendingEvent() {
        ...
        event.setEventType("NotificationDispatched");
    ...
    void marksEventPublishedWhenSendSucceeds() {
        ...
        when(streamBridge.send(eq("notificationEvents-out-0"), any(Message.class))).thenReturn(true);
    ...
    void incrementsRetryCountAndKeepsPendingWhenSendReturnsFalse() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("notificationEvents-out-0"), any(Message.class))).thenReturn(false);
