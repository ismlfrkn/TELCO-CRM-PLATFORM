...
import com.turkcell.usage.repository.OutboxEventRepository;
...
class OutboxEventPublisherTest {
    ...
    private OutboxEvent pendingEvent() {
        ...
        event.setEventType("QuotaExceeded");
    ...
    void marksEventPublishedWhenSendSucceeds() {
        ...
        when(streamBridge.send(eq("usageEvents-out-0"), any(Message.class))).thenReturn(true);
    ...
    void incrementsRetryCountAndKeepsPendingWhenSendReturnsFalse() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("usageEvents-out-0"), any(Message.class))).thenReturn(false);
