...
class OutboxEventPublisherTest {
    ...
    void marksEventPublishedWhenSendSucceeds() {
        ...
        when(streamBridge.send(eq("customerEvents-out-0"), any(Message.class))).thenReturn(true);
    ...
    void incrementsRetryCountAndKeepsPendingWhenSendReturnsFalse() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("customerEvents-out-0"), any(Message.class))).thenReturn(false);
    ...
    void incrementsRetryCountWhenSendThrows() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("customerEvents-out-0"), any(Message.class)))
