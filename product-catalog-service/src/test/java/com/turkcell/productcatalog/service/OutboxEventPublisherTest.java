...
class OutboxEventPublisherTest {
    ...
    private OutboxEvent pendingEvent() {
        ...
        event.setEventType("TariffCreated");
    ...
    void marksEventPublishedWhenSendSucceeds() {
        ...
        when(streamBridge.send(eq("catalogEvents-out-0"), any(Message.class))).thenReturn(true);
    ...
    void incrementsRetryCountAndKeepsPendingWhenSendReturnsFalse() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("catalogEvents-out-0"), any(Message.class))).thenReturn(false);
    ...
    void incrementsRetryCountWhenSendThrows() {
        OutboxEvent event = pendingEvent();
        mockPendingBatch(event);
        when(streamBridge.send(eq("catalogEvents-out-0"), any(Message.class)))
