...
docker compose --profile observability up -d       # + Zipkin/Prometheus/Grafana/ELK (bkz. Bölüm 16 — bu 6 container hiçbir domain servisinden gerçek veri almıyor, sadece ihtiyaç anında aç)
```
...
**Hâlâ implemente edilmemiş / kısmi (Bölüm 15'teki `--profile observability` notuna bkz.):**
- **Resilience4j** yalnızca `order-service`'in Customer/ProductCatalog Feign client'larında var; diğer 9 serviste ve payment-service'in mock PSP çağrısında hiç yok (spec "tüm dış çağrılarda" istiyor).
- **Tracing/JSON log/metrics tamamen bağlı değil**: `observability/` altındaki Prometheus/Grafana/ELK config'leri ve docker-compose'daki 6 container gerçek — ama hiçbir serviste `micrometer-tracing-bridge-otel`, `micrometer-registry-prometheus` veya JSON log encoder yok, yani bu yığın şu an veri almadan boşuna çalışıyor (bu yüzden varsayılan `docker compose up`'a dahil değil, bkz. Bölüm 15).
- **CI/CD ve Kubernetes hiç yok** — `.github/workflows` ve `k8s/` klasörü mevcut değil.
- **Tarife versiyonlama (FR-08)**: `product-catalog-service`'te dokümante edilmemiş bir `TariffVersion` entity'si var — muhtemelen kısmen implemente edilmiş, spec'teki veri modeliyle (Bölüm 9) karşılaştırılıp doğrulanmadan "tam çalışıyor" varsayılmasın.
